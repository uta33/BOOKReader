...
<p>最終更新日: 2026年8月4日</p>
<h2>取り扱うデータ</h2>
<p>匿名ユーザーID、Google連携時のGoogle固有IDとメールアドレス、本の書誌・読書ノート・抜き書き・添付画像・リンク・評価・読了情報、API利用件数、同期revision、匿名発行制御用の不可逆化したIPハッシュを取り扱います。音声入力を押した場合は、その間の音声と変換された文字を取り扱います。</p>
<p>端末へ取り込んだPDF/TXT/Markdown本文、端末内ファイルURI、音声キャッシュはクラウド同期しません。抜き書きへ利用者が添付した画像だけを、復元のため暗号化通信で非公開保存します。</p>
<p>ドッグイヤーの音声入力で取得した音声は、端末が選択している音声認識サービスによる一時的な文字変換にだけ使い、アプリ、Cloudflare、D1、R2には録音を保存しません。変換された文字は入力欄で確認でき、「保存する」を押した場合だけ抜き書きまたはコメントとして保存・同期します。</p>
<h2>外部サービス</h2>
<p>AI要約・クイズにはAnthropic、読み上げ・文字認識にはGoogle Cloud、認証にはGoogle OAuth、クラウド保存にはCloudflareを使用し、機能に必要な範囲で入力データを送信します。</p>
<p>ドッグイヤーの音声入力には、AndroidまたはiOSで端末が選択している音声認識サービスを使用します。認識が端末内で完結するか、サービス提供者へ音声が送信されるかは、端末、OS、選択中のサービス、言語モデルの設定によって異なります。</p>
<h2>保持と削除</h2>
