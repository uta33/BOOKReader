...
音声入力を押したときだけマイク権限を求め、Android／iOSが選択している標準の
音声認識サービスへ日本語認識を依頼する。認識途中の文字を入力欄へ反映し、確定結果は
開始前の既存文字の次の行へ追記する。録音データはアプリ、Worker、D1、R2のいずれにも
保存しない。利用者が内容を確認して保存した文字だけが通常のDogEar同期対象になる。

この機能はWorkerのAI APIとクォータを使わず、アプリ固有の従量課金は発生しない。
端末側サービスが端末内認識かネットワーク認識かを決めるため、オフライン動作は保証しない。
利用不能・権限拒否・無音・通信失敗は入力欄の近くに表示し、手入力は常に残す。
