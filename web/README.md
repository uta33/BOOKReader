...
## コンテンツの取り込み
- **AI生成**: 書名/トピックを指定 → Claude が要約台本を**ストリーミング生成**（生成中の文章がリアルタイムに表示され、そのまま読み始められます）。あわせて内容固有の**復習クイズ3問**も自動生成
- **台本取り込み**: 既成の要約を **`.md` / `.txt` ファイルでアップロード**（このときも復習クイズを自動生成）。Markdownの見出し（`#`）は章として認識し、箇条書き・強調などの記法は読み上げ用に自動で整えます

PDFの取り込みは行いません。

## 音声の保存（いつでも・オフラインでも聴ける）
- 生成した音声は文単位で **IndexedDB に永続保存**されます。リーダーの「音声を保存」で全文を
  一括生成・保存でき、以後はオフラインでも再生できます（初回保存時に `navigator.storage.persist()`
  を要求してブラウザの自動削除を防ぎます）。
- **再生速度は保存済み音声にそのまま反映**されます（`playbackRate` でクライアント適用するため、
  速度を変えても再生成は不要）。音声は声ごとに保存されます。
- 再生中は「現在位置＋先読み5文」だけを自動生成するので、TTSコストは聴いた分＋保存した分だけです。

## 構成
- フロント: Vite + React 19 + TypeScript + Zustand（localStorage 永続化）/ PWA
- 音声: Google Cloud TTS（サーバ経由・1.0x で合成）→ 無ければブラウザ内蔵の SpeechSynthesis →
  さらに無ければ時間送り、の順でフォールバック
- 生成モデル: `SUMMARY_MODEL` env（既定 `claude-sonnet-4-6`）
- バックエンド: `server/`（ローカル開発用 Express）と `api/`（Vercel サーバレス関数・ストリーミング対応）が
...
## 環境変数（サーバ側のみ）
- `ANTHROPIC_API_KEY` — Claude（要約台本・クイズ生成）。未設定時はサンプルにフォールバック
- `SUMMARY_MODEL` — 生成モデル（既定 `claude-sonnet-4-6`）
- `GOOGLE_TTS_API_KEY` — Google Cloud TTS。未設定時はブラウザ内蔵音声にフォールバック
- `API_PORT` — ローカルAPIのポート（既定 8787）

## 補足
- 要約生成は既定で `claude-sonnet-4-6`（コスト効率と品質のバランス）。`SUMMARY_MODEL` で変更可能。
  インストール済み SDK (0.70.x) の型が adaptive thinking 未対応のため `thinking` パラメータは
  指定していません（要約用途では品質に影響なし）。
